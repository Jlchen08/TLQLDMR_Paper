...
TLQLDMR improves on the strongest non-transfer baseline (HHO-SVR), raising $R^2$ from 0.9537 to 0.9553 and reducing RMSE from 7.603 to 7.476. Although the numerical margin is moderate, it is obtained under the most severe domain-shift setting with only 4{,}143 extreme-weather samples. The curve-level evidence in Figs.~\ref{fig:exp2_curves_a}--\ref{fig:exp2_curves_b} is consistent with the aggregate metrics: TLQLDMR tracks abrupt ramps and low-power transitions more closely, while several baselines show larger local deviations around rapid fluctuations. Among baselines, HHO-SVR remains the most competitive kernel method, RF-WPF remains the second-best overall alternative, and BRF-WPF does not surpass RF-WPF under the present small-sample extreme-weather regime. The observed gain of TLQLDMR is consistent with the combined effect of source-domain transfer, asymmetric target weighting, and cyclic wind-direction encoding. Because many extreme-weather periods contain near-zero power outputs, MAPE is numerically unstable and therefore RMSE and $R^2$ are treated as the principal deterministic indicators.
...
\begin{table*}[ht]
\centering
\caption{Interval prediction results on the extreme weather test set of Farm~6 at 95\% confidence level ($\alpha = 0.05$). Baseline models are sorted by CWC in ascending order, while TLQLDMR is placed in the last row for direct comparison. Best results per column are highlighted in bold.}
\setlength{\tabcolsep}{4mm}
\begin{tabular}{lccccc}
\toprule
...
\begin{table*}[ht]
\centering
\caption{Full interval metrics under injected test-input noise on the Farm~6 extreme-weather test set. For PICP, values near 0.95 are preferred; for MPIW, PINAW, CWC, and Winkler, lower values are preferred.}
...
To validate the contribution of the three core components (transfer learning, LDMR variance regularization, and MMD feature alignment), this ablation experiment evaluates structurally reduced variants on both point and interval tasks.
...
\textbf{Transfer learning improves deterministic forecasting under severe shift.} Experiment~2 shows consistent gains in RMSE and $R^2$, and curve-level evidence confirms better tracking during abrupt peak-valley transitions.
