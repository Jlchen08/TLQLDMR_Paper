...
The target domain is split via random shuffling with a fixed seed to ensure reproducibility. The 90\%/10\% target split is selected to maximize training signal under scarce extreme samples while retaining an independent hold-out subset for evaluation. All baseline models are trained exclusively on target-domain training data, whereas TLQLDMR additionally leverages source-domain samples through transfer learning.
...
\midrule
\textbf{TLQLDMR (Ours)} & \textbf{0.9584} & \textbf{0.9560} & \textbf{0.9603} \\
\bottomrule
\end{tabular}
\label{tab:exp2_noise}
\end{table}

The observation confirms that while all models experience performance degradation as the noise level increases (SNR decreases), TLQLDMR consistently maintains the highest $R^2$ across all testing conditions. In particular, TLQLDMR remains above 0.94 even at 10\,dB, while BRF-WPF exhibits the steepest degradation, indicating higher sensitivity to measurement perturbations. The transfer learning framework, reinforced by Large Margin Distribution regularization, effectively mitigates overfitting to noisy target samples and demonstrates the strongest noise resilience among the comparative models.
\FloatBarrier
%---------------- Experiment 3 --------------------%
...
To assess interval robustness under sensor perturbation, Gaussian noise is injected only into test inputs at SNR $\in \{60,40,30\}$ dB, while calibration parameters remain fixed to preserve comparability across models. For TLQLDMR, a conservative scaling multiplier (1.15) is used to maintain coverage. Unlike the previous single-metric presentation, Table~\ref{tab:exp3_noise} reports the full set of interval metrics.

\begin{table*}[ht]
\centering
\caption{Full interval metrics under injected test-input noise on the Farm~6 extreme-weather test set. Models are sorted consistently with Table~\ref{tab:exp3_results}. For PICP, values near 0.95 are preferred; for MPIW, PINAW, CWC, and Winkler, lower values are preferred. Best CWC results are highlighted in bold.}
...
\midrule
\multirow{3}{*}{NuSVR-CI}
  & 30 & 0.8386 & 19.970 & 0.2077 & 0.2856 & 54.922 \\
  & 40 & 0.8361 & 19.970 & 0.2077 & 0.2860 & 54.623 \\
  & 60 & 0.8434 & 19.970 & 0.2077 & 0.2849 & 54.588 \\
...
\multirow{3}{*}{\textbf{TLQLDMR (Ours)}}
  & 30 & 0.9542 & 26.709 & 0.2778 & \textbf{0.2778} & 36.307 \\
  & 40 & 0.9566 & 26.713 & 0.2779 & \textbf{0.2779} & 36.034 \\
  & 60 & 0.9566 & 26.715 & 0.2779 & \textbf{0.2779} & 36.083 \\
\bottomrule
\end{tabular}
...
Critically, the LDMR-Quantile (SymVar) variant—which replaces asymmetric variance with symmetric variance minimization—exhibits degraded performance on both point prediction (RMSE: 7.868 vs. 7.476) and interval prediction (CWC: 0.3029 vs. 0.2683, PINAW: 0.3029 vs. 0.2683). This degradation empirically validates our theoretical analysis in Section~\ref{novel}: symmetric variance minimization, while effective for mean regression, conflicts with the asymmetric residual distribution required for accurate quantile estimation. The symmetric regularizer forces errors toward a centered, Gaussian-like distribution, thereby degrading interval calibration at non-median quantiles. This comparison confirms that asymmetric variance regularization is not merely a technical refinement but a theoretically necessary modification for extending LDMR to quantile regression tasks.
